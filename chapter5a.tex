...
% i procedurą przeglądu (odpowiednio, przeglądem).
...
%
%\begin{algorithm}
%	\caption{\todo{name and label me}}
%	\begin{algorithmic}[1]
%	\Params{
%		$X_1$, $X_2$, $\ldots$, $X_{p}$ \\
%		$I$ \\
%		$statystyka$ \\
%		\Comment{\raggedleft Parametry zdefiniowane w ten sam sposób co dla procedury eksploracji (patrz: algorytm \ref{algorithm:explore})}
%	}
%	\Input{
%		$cialo$ \Comment{\raggedleft Ciało eksperymentu, zdefiniowane w ten sam sposób co dla procedury eksploracji
%		}
%	}
%	\Output{
%		Najlepsza konfiguracja znaleziona podczas przeglądu zupełnego, tj. $p$ wartości $x_{i, j}$ o różnych indeksach $i$
%	}
%	\Start
%	\Operator{przeglad}{$cialo$}
%		\State \Return \opName{okresl}($cialo$, $(X_1, X_2, \ldots, X_p)$, $()$)
%	\EndOperator
%%	\Statex \hrulefill
%	\algrule
%	\Input{
%		$cialo$ \Comment{\raggedleft Ciało eksperymentu, zdefiniowane w ten sam sposób co dla operatora \opName{przeglad}} \\
%		$(X_n, X_{n+1}, \ldots, X_p)$ \Comment{Końcowy podciąg zestawu zbiorów wartości parametrów, który należy zbadać} \\
%		$(p_1, p_2, \ldots, p_{n-1})$ \Comment{Częściowa konfiguracja, dla której szukane są dalsze parametry z zestawu $(X_n, X_{n+1}, \ldots, X_p)$}
%	}
%	\Output{
%		Najlepsza konfiguracja znaleziona dla zadanego ciągu $(p_1, p_2, \ldots, p_{n-1})$
%	}
%	\Start
%	\Operator{okresl}{$cialo$, $(X_n, X_{n+1}, \ldots, X_p)$, $(p_1, p_2, \ldots, p_{n-1})$}
%		\Var $statystyki = [NULL, NULL, \ldots]$ \Comment{Tablica o rozmiarze $|X_{n}|$}
%		\Var $konfiguracje = [NULL, NULL, \ldots]$ \Comment{J.w.}
%		\For{$ i \gets 1$ \To $|X_{n}|$}
%			\If{$n = p$} \Comment{Określono wszystkie parametry poza jednym}
%				\State $konfiguracje[i] \gets (p_1, p_2, \ldots, p_{n-1}, x_{n, i})$ 
%			\Else
%				\Var $zbiory \gets (X_{n+1}, X_{n+2}, \ldots, X_p)$
%				\Var $parametry \gets (p_1, p_2, \ldots, p_{n-1}, x_{n, i})$ 			
%				\State $konfiguracje[i] \gets \opName{okresl}(cialo, zbiory, parametry)$
%			\EndIf
%			\Var $wyniki \gets \emptyset$
%			\For{$powtorzenie \gets 1$ \To $I$}
%				\State $wyniki \gets wyniki \cup \{ cialo(konfiguracje[i]) \}$
%			\EndFor
%			\State $statystyki[i] \gets statystyka(wyniki)$
%		\EndFor
%		\Var $najlepszaKonfiguracja \gets \argmin_{i} statystyki[i]$
%		\State \Return $konfiguracje[najlepszaKonfiguracja]$
%	\EndOperator
%	\end{algorithmic}
%\end{algorithm}
%
