...
\subsection{Implementacja problemu komiwojażera}
...
Dodatkowo, aby pokierować heurystyką tak, żeby ograniczenie na łączny rozmiar wybranych przedmiotów było spełnione, wprowadzono funkcję kary, polegającą na dodaniu do funkcji oceny iloczynu łącznego kosztu przedmiotów i różnicy między sumą ich obiętości, a maksymalnym rozmiarem plecaka.
...
Dla tak przedstawionego zagadnienia znane jest optimum, wynoszące -1137, co oznacza wybrane przedmioty mają łączny koszt 1137 i ograniczenie na rozmiar nie jest przekroczone.
...
W zależności od tego, czy łączna objętość wybranych przedmiotów była większa, czy mniejsza, niż rozmiar plecaka, prawdopodobieństwa tego, że bit zostanie odwrócony (tzn. 0 zostanie zamienione na 1 lub na odwrót) było różne.

Zanim działanie tego operatora zostanie zdefiniowane formalnie, należy opisać pomocniczą funkcję $wybierz(\range{a}{b}, x)$, zwracającą punkt leżący w przedziale $\range{a}{b}$ w odległości od obu punktów określonej przez argument $x$.
Jej definicja jest zapisana w sygnaturze \ref{signature:wybierz}.

\begin{signature}
	\caption{Funkcja $wybierz(\range{a}{b}, x)$ \label{signature:wybierz}}
	\begin{align}
		wybierz: &\numberSet{R}^2 \times \range{0}{1} \rightarrow \numberSet{R} \\
		wybierz(\range{a}{b}, x) =& a + (b-a) \times x
	\end{align}
\end{signature}
...
\begin{align}
	\label{line:inverse}
	P = wybierz(przedzial, czynnik)
\end{align}

Sposób obliczania wartości $czynnik$ jest zależny od sytuacji. 
Jeżeli obserwowany jest niedomiar, to jego wartość jest różnicą rozmiaru plecaka i sumy rozmiarów wybranych przedmiotów podzieloną przez rozmiar plecaka ($\frac{W - \sum_{i=1}^p (x_i \times w_i)}{W}$).
Jeżeli natomiast występuje nadmiar, to jest on tożsamy z wartością funkcji $nadmiar(x)$ (patrz: linia \ref{line:knapsack_eval_stop}) podzieloną przez sumę różnicę sumy wszystkich wag i rozmiaru plecaka ($\frac{nadmiar(x)}{(\sum_{i=1}^p w_i) - W}$).
