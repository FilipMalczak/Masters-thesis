...
\begin{signature}
	...
	\begin{align}
		...
		q = & \param{prawdKrzyzowania} \times \param{rozmiarPopulacji}\\
		T \gets &\left\{ t_0, t_1, \ldots t_{\param{rozmiarPopulacji}-1} \right\} \\
...
\begin{algorithm}
	\caption{Schemat działania wybranych \todo{rozwiązań literaturowych} \label{algorithm:common_genSel} }
	\begin{algorithmic}[1]
		\Input{
			$populacja$ \Comment{Populacja wyjściowa}
		}
		...
		\Params{
			...
			$\param{plecMaZnaczenie}$ \Comment{\raggedleft Zmienna logiczna określająca, czy rodzice muszą różnić się płcią}\\
		...
		\label{line:stdGenSelChoose_while}
			...
			\Var $rodzic2 \gets opWyboru1(kandydaci1 \setminus \left\{ rodzic1 \right\}) $
