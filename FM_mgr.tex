...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Operator{opRuletkowy}{$populacja$}
			\Var \label{line:roulette_sort} $posortowane \gets$ $populacja$ posortowana malejąco po wartości funkcji $\param{waga}$
			\Var $sumaWag \gets \sum_{s \in populacja} \param{waga}(s)$
			\Var $k \gets 0$
			\Var $wybrane \gets random(\range{0}{sumaWag})$
			...
			\Comment{w kolejności określonej w linii \ref{line:roulette_sort}}
				\If{$wybrane \geq k \wedge wybrane < k + \param{waga}(s)$}
					\State \Return $s$
				\Else
					\State $k \gets k + \param{waga}(s)$
				\EndIf
			\EndFor
			\State \Return ostatni element $posortowane$
		\EndOperator
	\end{algorithmic}
\end{algorithm}
