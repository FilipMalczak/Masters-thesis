...
\todo{osobnik musi zawierać ceche plci, dodać do sygnatury S-DSEA = S-EA x G}
...
\begin{signature}
	...
	W niniejszej pracy przyjęto co najwyżej 2 płcie, ze zbiorem $\important{G}$ określonym jako $\important{G} = \{ \female, \male \}$
\end{signature}
...
\fixme{zmienna, zmienne, zmiennej, etc}

Działanie operatora rozpoczyna się od deklaracji i inicjalizacji zmiennych pomocniczych.
Zmienna $rodzice$ przyjmuje jako wartość pusty zbiór (linia \ref{line:stdGenSelChoose_init_parent}).
Dodatkowo, deklarujemy 2 zmienne przechowujące kandydatów na każdego z rodziców (linie \ref{line:stdGenSelChoose_init_candidates_begin}-\ref{line:stdGenSelChoose_init_candidates_end}).
...
Drugi z rodziców jest wybierany ze zbioru $\param{kandydaci2}$ za pomocą operatora $\param{opWyboru2}$  (linia \ref{line:stdGenSelChoose_choose_end}), \fixme{unikając sytuacji} w której oboje rodziców byłoby tym samym osobnikiem (mogłoby się tak zdarzyć, jeżeli $\param{plecMaZnaczenie} = \bot$, więc oboje z nich byłoby wybieranych z całej populacji).
...
W praktyce lepsze wyniki \todo{na pewno?} osiągano używając dwóch różnych operatorów wyboru, jednak sytuacje w której użyto tego samego operatora dla obu płci, lub któryś z nich był operatorem losowym są warte zbadania.
...
W momencie, w którym osobniki alfa są czymś zajęte, pewien odsetek osobników tej samej płci ma szansę na rozmnożenie się doprowadzić do wydania potomstwa.
...
Zrealizowano to definiując realizację operatora selekcji płciowej, nazywaną operatorem haremowym, opisaną w algorytmie \ref{algorithm:harem}.
...
Operator taki jest parametryzowany przy pomocy 3 operatorów wyboru: $\param{opWyboruAlfa}$ służącego do wyboru osobników alfa, $\param{opWyboruBeta}$ służącego do wyboru osobników beta oraz $\param{opWyboruPartnerów}$ wykorzystywanego do wybory partnerów dla osobników alfa i beta.
Ponadto, operator haremowy ma 2 parametry liczbowe: $\param{liczbaAlfa}$ określający liczbę osobników alfa wybieranych w pojedynczym pokoleniu i $\param{wspBeta}$, czyli współczynnik określający stosunek liczby par rodziców z których jeden jest osobnikiem alfa do liczby wszystkich par rodziców 

Działanie operatora zaczyna się od inicjalizacji zmiennej $rodzice$ na pusty zbiór oraz rozdzielenia populacji na zbiory $kandydaci1$ i $kandydaci2$, zawierające osobniki różnych płci X.
Następnie obliczane są wartości $liczbaZbiorowRodzicow$, czyli całkowita liczba par rodziców zwracanych przez operator  X, $liczbaBeta$, czyli liczba osobników beta biorących udział w rozmnażaniu  X, oraz $perAlfa$, czyli liczba par rodziców przypadających na jednego osobnika alfa X.

Kolejnym krokiem jest wygenerowanie par rodziców zawierających osobnika alfa.
$\param{liczbaAfa}$ razy powtarzane jest X wybranie kolejnego takiego osobnika $alfa$ X i usunięcie go ze zbioru kandydatów danej płci X.
Dzięki temu żaden osobnik alfa nie zostanie wybrany dwukrotnie, ani nie pokryje się z którymś z osobników beta.
Dla danego osobnika alfa wybierany jest $perAlfa$ X partnerów $partner$, a tak określone pary rodziców są dołączane do zbioru $rodzice$ X

W dalszej kolejności wybierane jest $liczbaBeta$ osobników beta X, dla których dobierani są partnerzy $partner$ X. Tak wybrane pary również dołączane są do zbioru $rodzice$.

Operator zwraca zmienną $rodzice$.

\todo{dodać labele, X zamienić na ref do nich}
